...
> 📌 **当前版本定位**：实现的是**实时跌倒检测**（判断"此刻是否正在跌倒"）。
> 若要把本项目推进为研究论文，需重构为**跌倒提前预警（pre-impact anticipation）**——
> 完整任务清单见 [`docs/论文改进路线图.md`](docs/论文改进路线图.md)。
...
| 文档 | 内容 |
|------|------|
| [`docs/论文改进路线图.md`](docs/论文改进路线图.md) | **研究升级 TODO**：阻断性问题、评估规范化、方法创新方向、目标会议与决策点 |
